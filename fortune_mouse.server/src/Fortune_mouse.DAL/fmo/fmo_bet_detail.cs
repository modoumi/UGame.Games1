...
namespace Fortune_mouse.DAL
{
	...
	/// <summary>
	/// 下注表
	/// 【表 fmo_bet_detail 的实体类】
	/// </summary>
	[DataContract]
	[Obsolete]
	public class Fmo_bet_detailEO : IRowMapper<Fmo_bet_detailEO>
	{
		/// <summary>
		/// 构造函数 
		/// </summary>
		public Fmo_bet_detailEO()
		{
			this.ChipsID = 0;
			this.BetAmount = 0;
		...
		/// <return>实体对象</return>
		public Fmo_bet_detailEO MapRow(IDataReader reader)
		{
		...
		/// <param name = "reader">只进结果集流</param>
		/// <return>实体对象</return>
		public static Fmo_bet_detailEO MapDataReader(IDataReader reader)
		{
		    Fmo_bet_detailEO ret = new Fmo_bet_detailEO();
	...
	/// <summary>
	/// 下注表
	/// 【表 fmo_bet_detail 的操作类】
	/// </summary>
	[Obsolete]
	public class Fmo_bet_detailMO : MySqlTableMO<Fmo_bet_detailEO>
	{
		/// <summary>
		/// 表名
		/// </summary>
	    public override string TableName { get; set; } = "`fmo_bet_detail`";
		...
		public Fmo_bet_detailMO(MySqlDatabase database) : base(database) { }
		/// <summary>
		/// 构造函数
		/// </summary>
		/// <param name = "connectionStringName">配置文件.config中定义的连接字符串名称</param>
		public Fmo_bet_detailMO(string connectionStringName = null) : base(connectionStringName) { }
	    /// <summary>
	    /// 构造函数
	    /// </summary>
	    /// <param name="connectionString">数据库连接字符串，如server=192.168.1.1;database=testdb;uid=root;pwd=root</param>
	    /// <param name="commandTimeout">CommandTimeout时间</param>
	    public Fmo_bet_detailMO(string connectionString, int commandTimeout) : base(connectionString, commandTimeout) { }
		...
		public override int Add(Fmo_bet_detailEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_); 
		}
		public override async Task<int> AddAsync(Fmo_bet_detailEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_); 
		}
	    private void RepairAddData(Fmo_bet_detailEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = useIgnore_ ? "INSERT IGNORE" : "INSERT";
			sql_ += $" INTO {TableName} (`BetID`, `ChipsID`, `AppID`, `OperatorID`, `UserID`, `BetAmount`, `BalanceAmount`, `IsWin`, `WinAmount`, `TotalMultip`, `TriggerSM`, `TriggerSE`, `TriggerMaxMultip`, `HistoryLine`, `HistoryMultip`, `RewardDetails`, `SlotEleArray`, `BonusInfo`, `UserFlag`, `OrderStatus`, `RecDate`) VALUE (@BetID, @ChipsID, @AppID, @OperatorID, @UserID, @BetAmount, @BalanceAmount, @IsWin, @WinAmount, @TotalMultip, @TriggerSM, @TriggerSE, @TriggerMaxMultip, @HistoryLine, @HistoryMultip, @RewardDetails, @SlotEleArray, @BonusInfo, @UserFlag, @OrderStatus, @RecDate);";
		...
		public int AddByBatch(IEnumerable<Fmo_bet_detailEO> items, int batchCount, TransactionManager tm_ = null)
		{
			var ret = 0;
	    ...
	    public async Task<int> AddByBatchAsync(IEnumerable<Fmo_bet_detailEO> items, int batchCount, TransactionManager tm_ = null)
	    {
	        var ret = 0;
	        foreach (var sql in BuildAddBatchSql(items, batchCount))
	        {
	    ...
	    private IEnumerable<string> BuildAddBatchSql(IEnumerable<Fmo_bet_detailEO> items, int batchCount)
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`BetID`, `ChipsID`, `AppID`, `OperatorID`, `UserID`, `BetAmount`, `BalanceAmount`, `IsWin`, `WinAmount`, `TotalMultip`, `TriggerSM`, `TriggerSE`, `TriggerMaxMultip`, `HistoryLine`, `HistoryMultip`, `RewardDetails`, `SlotEleArray`, `BonusInfo`, `UserFlag`, `OrderStatus`, `RecDate`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"({item.BetID},{item.ChipsID},'{item.AppID}','{item.OperatorID}','{item.UserID}',{item.BetAmount},{item.BalanceAmount},{item.IsWin},{item.WinAmount},{item.TotalMultip},{item.TriggerSM},{item.TriggerSE},{item.TriggerMaxMultip},'{item.HistoryLine}','{item.HistoryMultip}','{item.RewardDetails}','{item.SlotEleArray}','{item.BonusInfo}',{item.UserFlag},{item.OrderStatus},'{item.RecDate.ToString("yyyy-MM-dd HH:mm:ss")}'),");
				if (count == batchCount)
				{
					count = 0;
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Remove(Fmo_bet_detailEO item, TransactionManager tm_ = null)
		{
			return RemoveByPK(item.BetID, tm_);
		}
		public async Task<int> RemoveAsync(Fmo_bet_detailEO item, TransactionManager tm_ = null)
		{
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "isWin">是否中奖(奖池是否充足)</param>
		/// <param name="tm_">事务管理对象</param>
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "winAmount">所有奖励总和</param>
		/// <param name="tm_">事务管理对象</param>
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "triggerSM">是否触发特殊玩法</param>
		/// <param name="tm_">事务管理对象</param>
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "triggerSE">触发特殊玩法是否中奖</param>
		/// <param name="tm_">事务管理对象</param>
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "historyLine">中奖线记录</param>
		/// <param name="tm_">事务管理对象</param>
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "historyMultip">中奖线对应的倍率</param>
		/// <param name="tm_">事务管理对象</param>
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "bonusInfo">特殊玩法数据结构</param>
		/// <param name="tm_">事务管理对象</param>
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(Fmo_bet_detailEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAsync(Fmo_bet_detailEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutData(Fmo_bet_detailEO item, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `BetID` = @BetID, `ChipsID` = @ChipsID, `AppID` = @AppID, `OperatorID` = @OperatorID, `UserID` = @UserID, `BetAmount` = @BetAmount, `BalanceAmount` = @BalanceAmount, `IsWin` = @IsWin, `WinAmount` = @WinAmount, `TotalMultip` = @TotalMultip, `TriggerSM` = @TriggerSM, `TriggerSE` = @TriggerSE, `TriggerMaxMultip` = @TriggerMaxMultip, `HistoryLine` = @HistoryLine, `HistoryMultip` = @HistoryMultip, `RewardDetails` = @RewardDetails, `SlotEleArray` = @SlotEleArray, `BonusInfo` = @BonusInfo, `UserFlag` = @UserFlag, `OrderStatus` = @OrderStatus WHERE `BetID` = @BetID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@BetID", item.BetID, MySqlDbType.Int64),
				Database.CreateInParameter("@ChipsID", item.ChipsID, MySqlDbType.Int32),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(IEnumerable<Fmo_bet_detailEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public async Task<int> PutAsync(IEnumerable<Fmo_bet_detailEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public bool Set(Fmo_bet_detailEO item, TransactionManager tm = null)
		{
			bool ret = true;
		...
		public async Task<bool> SetAsync(Fmo_bet_detailEO item, TransactionManager tm = null)
		{
			bool ret = true;
			if(GetByPK(item.BetID) == null)
			{
		...
		/// <return></return>
		public Fmo_bet_detailEO GetByPK(long betID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(betID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return Database.ExecSqlSingle(sql_, paras_, tm_, Fmo_bet_detailEO.MapDataReader);
		}
		public async Task<Fmo_bet_detailEO> GetByPKAsync(long betID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(betID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return await Database.ExecSqlSingleAsync(sql_, paras_, tm_, Fmo_bet_detailEO.MapDataReader);
		}
		...
		public List<Fmo_bet_detailEO> GetByChipsID(int chipsID)
		{
			return GetByChipsID(chipsID, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByChipsIDAsync(int chipsID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByChipsID(int chipsID, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByChipsIDAsync(int chipsID, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByChipsID(int chipsID, int top_)
		{
			return GetByChipsID(chipsID, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByChipsIDAsync(int chipsID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByChipsID(int chipsID, int top_, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByChipsIDAsync(int chipsID, int top_, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByChipsID(int chipsID, string sort_)
		{
			return GetByChipsID(chipsID, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByChipsIDAsync(int chipsID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByChipsID(int chipsID, string sort_, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByChipsIDAsync(int chipsID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByChipsID(int chipsID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`ChipsID` = @ChipsID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@ChipsID", chipsID, MySqlDbType.Int32));
			return Database.ExecSqlList(sql_, paras_, tm_, Fmo_bet_detailEO.MapDataReader);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByChipsIDAsync(int chipsID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`ChipsID` = @ChipsID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@ChipsID", chipsID, MySqlDbType.Int32));
			return await Database.ExecSqlListAsync(sql_, paras_, tm_, Fmo_bet_detailEO.MapDataReader);
		}
		...
		/// /// <param name = "appID">appid</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByAppID(string appID)
		{
			return GetByAppID(appID, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByAppIDAsync(string appID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByAppID(string appID, TransactionManager tm_)
		{
			return GetByAppID(appID, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByAppIDAsync(string appID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByAppID(string appID, int top_)
		{
			return GetByAppID(appID, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByAppIDAsync(string appID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByAppID(string appID, int top_, TransactionManager tm_)
		{
			return GetByAppID(appID, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByAppIDAsync(string appID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByAppID(string appID, string sort_)
		{
			return GetByAppID(appID, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByAppIDAsync(string appID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByAppID(string appID, string sort_, TransactionManager tm_)
		{
			return GetByAppID(appID, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByAppIDAsync(string appID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByAppID(string appID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByAppIDAsync(string appID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`AppID` = @AppID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@AppID", appID, MySqlDbType.VarChar));
		...
		/// /// <param name = "operatorID">运营商编码</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOperatorID(string operatorID)
		{
			return GetByOperatorID(operatorID, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOperatorIDAsync(string operatorID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOperatorID(string operatorID, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOperatorIDAsync(string operatorID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOperatorID(string operatorID, int top_)
		{
			return GetByOperatorID(operatorID, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOperatorIDAsync(string operatorID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOperatorID(string operatorID, int top_, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOperatorIDAsync(string operatorID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOperatorID(string operatorID, string sort_)
		{
			return GetByOperatorID(operatorID, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOperatorIDAsync(string operatorID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOperatorID(string operatorID, string sort_, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOperatorIDAsync(string operatorID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOperatorID(string operatorID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByOperatorIDAsync(string operatorID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`OperatorID` = @OperatorID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@OperatorID", operatorID, MySqlDbType.VarChar));
		...
		/// /// <param name = "userID">用户Id</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserID(string userID)
		{
			return GetByUserID(userID, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserIDAsync(string userID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserID(string userID, TransactionManager tm_)
		{
			return GetByUserID(userID, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserIDAsync(string userID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserID(string userID, int top_)
		{
			return GetByUserID(userID, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserIDAsync(string userID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserID(string userID, int top_, TransactionManager tm_)
		{
			return GetByUserID(userID, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserIDAsync(string userID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserID(string userID, string sort_)
		{
			return GetByUserID(userID, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserIDAsync(string userID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserID(string userID, string sort_, TransactionManager tm_)
		{
			return GetByUserID(userID, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserIDAsync(string userID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserID(string userID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByUserIDAsync(string userID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserID` = @UserID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar));
		...
		/// /// <param name = "betAmount">实际下注金额</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBetAmount(long betAmount)
		{
			return GetByBetAmount(betAmount, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBetAmountAsync(long betAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBetAmount(long betAmount, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBetAmountAsync(long betAmount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBetAmount(long betAmount, int top_)
		{
			return GetByBetAmount(betAmount, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBetAmountAsync(long betAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBetAmount(long betAmount, int top_, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBetAmountAsync(long betAmount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBetAmount(long betAmount, string sort_)
		{
			return GetByBetAmount(betAmount, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBetAmountAsync(long betAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBetAmount(long betAmount, string sort_, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBetAmountAsync(long betAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBetAmount(long betAmount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByBetAmountAsync(long betAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`BetAmount` = @BetAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@BetAmount", betAmount, MySqlDbType.Int64));
		...
		/// /// <param name = "balanceAmount">下注后金额</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBalanceAmount(long balanceAmount)
		{
			return GetByBalanceAmount(balanceAmount, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBalanceAmountAsync(long balanceAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBalanceAmount(long balanceAmount, TransactionManager tm_)
		{
			return GetByBalanceAmount(balanceAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBalanceAmountAsync(long balanceAmount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBalanceAmount(long balanceAmount, int top_)
		{
			return GetByBalanceAmount(balanceAmount, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBalanceAmountAsync(long balanceAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBalanceAmount(long balanceAmount, int top_, TransactionManager tm_)
		{
			return GetByBalanceAmount(balanceAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBalanceAmountAsync(long balanceAmount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBalanceAmount(long balanceAmount, string sort_)
		{
			return GetByBalanceAmount(balanceAmount, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBalanceAmountAsync(long balanceAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBalanceAmount(long balanceAmount, string sort_, TransactionManager tm_)
		{
			return GetByBalanceAmount(balanceAmount, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBalanceAmountAsync(long balanceAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBalanceAmount(long balanceAmount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByBalanceAmountAsync(long balanceAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`BalanceAmount` = @BalanceAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@BalanceAmount", balanceAmount, MySqlDbType.Int64));
		...
		/// /// <param name = "isWin">是否中奖(奖池是否充足)</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByIsWin(int isWin)
		{
			return GetByIsWin(isWin, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByIsWinAsync(int isWin)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByIsWin(int isWin, TransactionManager tm_)
		{
			return GetByIsWin(isWin, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByIsWinAsync(int isWin, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByIsWin(int isWin, int top_)
		{
			return GetByIsWin(isWin, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByIsWinAsync(int isWin, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByIsWin(int isWin, int top_, TransactionManager tm_)
		{
			return GetByIsWin(isWin, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByIsWinAsync(int isWin, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByIsWin(int isWin, string sort_)
		{
			return GetByIsWin(isWin, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByIsWinAsync(int isWin, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByIsWin(int isWin, string sort_, TransactionManager tm_)
		{
			return GetByIsWin(isWin, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByIsWinAsync(int isWin, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByIsWin(int isWin, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByIsWinAsync(int isWin, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`IsWin` = @IsWin", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@IsWin", isWin, MySqlDbType.Int32));
		...
		/// /// <param name = "winAmount">所有奖励总和</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByWinAmount(long winAmount)
		{
			return GetByWinAmount(winAmount, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByWinAmountAsync(long winAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByWinAmount(long winAmount, TransactionManager tm_)
		{
			return GetByWinAmount(winAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByWinAmountAsync(long winAmount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByWinAmount(long winAmount, int top_)
		{
			return GetByWinAmount(winAmount, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByWinAmountAsync(long winAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByWinAmount(long winAmount, int top_, TransactionManager tm_)
		{
			return GetByWinAmount(winAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByWinAmountAsync(long winAmount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByWinAmount(long winAmount, string sort_)
		{
			return GetByWinAmount(winAmount, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByWinAmountAsync(long winAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByWinAmount(long winAmount, string sort_, TransactionManager tm_)
		{
			return GetByWinAmount(winAmount, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByWinAmountAsync(long winAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByWinAmount(long winAmount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByWinAmountAsync(long winAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`WinAmount` = @WinAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@WinAmount", winAmount, MySqlDbType.Int64));
		...
		/// /// <param name = "totalMultip">中奖总倍数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTotalMultip(int totalMultip)
		{
			return GetByTotalMultip(totalMultip, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTotalMultipAsync(int totalMultip)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTotalMultip(int totalMultip, TransactionManager tm_)
		{
			return GetByTotalMultip(totalMultip, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTotalMultipAsync(int totalMultip, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTotalMultip(int totalMultip, int top_)
		{
			return GetByTotalMultip(totalMultip, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTotalMultipAsync(int totalMultip, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTotalMultip(int totalMultip, int top_, TransactionManager tm_)
		{
			return GetByTotalMultip(totalMultip, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTotalMultipAsync(int totalMultip, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTotalMultip(int totalMultip, string sort_)
		{
			return GetByTotalMultip(totalMultip, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTotalMultipAsync(int totalMultip, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTotalMultip(int totalMultip, string sort_, TransactionManager tm_)
		{
			return GetByTotalMultip(totalMultip, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTotalMultipAsync(int totalMultip, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTotalMultip(int totalMultip, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByTotalMultipAsync(int totalMultip, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`TotalMultip` = @TotalMultip", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@TotalMultip", totalMultip, MySqlDbType.Int32));
		...
		/// /// <param name = "triggerSM">是否触发特殊玩法</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSM(int triggerSM)
		{
			return GetByTriggerSM(triggerSM, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSMAsync(int triggerSM)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSM(int triggerSM, TransactionManager tm_)
		{
			return GetByTriggerSM(triggerSM, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSMAsync(int triggerSM, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSM(int triggerSM, int top_)
		{
			return GetByTriggerSM(triggerSM, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSMAsync(int triggerSM, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSM(int triggerSM, int top_, TransactionManager tm_)
		{
			return GetByTriggerSM(triggerSM, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSMAsync(int triggerSM, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSM(int triggerSM, string sort_)
		{
			return GetByTriggerSM(triggerSM, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSMAsync(int triggerSM, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSM(int triggerSM, string sort_, TransactionManager tm_)
		{
			return GetByTriggerSM(triggerSM, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSMAsync(int triggerSM, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSM(int triggerSM, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSMAsync(int triggerSM, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`TriggerSM` = @TriggerSM", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@TriggerSM", triggerSM, MySqlDbType.Int32));
		...
		/// /// <param name = "triggerSE">触发特殊玩法是否中奖</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSE(int triggerSE)
		{
			return GetByTriggerSE(triggerSE, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSEAsync(int triggerSE)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSE(int triggerSE, TransactionManager tm_)
		{
			return GetByTriggerSE(triggerSE, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSEAsync(int triggerSE, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSE(int triggerSE, int top_)
		{
			return GetByTriggerSE(triggerSE, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSEAsync(int triggerSE, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSE(int triggerSE, int top_, TransactionManager tm_)
		{
			return GetByTriggerSE(triggerSE, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSEAsync(int triggerSE, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSE(int triggerSE, string sort_)
		{
			return GetByTriggerSE(triggerSE, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSEAsync(int triggerSE, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSE(int triggerSE, string sort_, TransactionManager tm_)
		{
			return GetByTriggerSE(triggerSE, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSEAsync(int triggerSE, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerSE(int triggerSE, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerSEAsync(int triggerSE, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`TriggerSE` = @TriggerSE", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@TriggerSE", triggerSE, MySqlDbType.Int32));
		...
		/// /// <param name = "triggerMaxMultip">中五条线触发10倍率</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerMaxMultip(int triggerMaxMultip)
		{
			return GetByTriggerMaxMultip(triggerMaxMultip, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerMaxMultipAsync(int triggerMaxMultip)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerMaxMultip(int triggerMaxMultip, TransactionManager tm_)
		{
			return GetByTriggerMaxMultip(triggerMaxMultip, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerMaxMultipAsync(int triggerMaxMultip, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerMaxMultip(int triggerMaxMultip, int top_)
		{
			return GetByTriggerMaxMultip(triggerMaxMultip, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerMaxMultipAsync(int triggerMaxMultip, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerMaxMultip(int triggerMaxMultip, int top_, TransactionManager tm_)
		{
			return GetByTriggerMaxMultip(triggerMaxMultip, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerMaxMultipAsync(int triggerMaxMultip, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerMaxMultip(int triggerMaxMultip, string sort_)
		{
			return GetByTriggerMaxMultip(triggerMaxMultip, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerMaxMultipAsync(int triggerMaxMultip, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerMaxMultip(int triggerMaxMultip, string sort_, TransactionManager tm_)
		{
			return GetByTriggerMaxMultip(triggerMaxMultip, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerMaxMultipAsync(int triggerMaxMultip, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByTriggerMaxMultip(int triggerMaxMultip, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByTriggerMaxMultipAsync(int triggerMaxMultip, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`TriggerMaxMultip` = @TriggerMaxMultip", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@TriggerMaxMultip", triggerMaxMultip, MySqlDbType.Int32));
		...
		/// /// <param name = "historyLine">中奖线记录</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryLine(string historyLine)
		{
			return GetByHistoryLine(historyLine, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryLineAsync(string historyLine)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryLine(string historyLine, TransactionManager tm_)
		{
			return GetByHistoryLine(historyLine, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryLineAsync(string historyLine, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryLine(string historyLine, int top_)
		{
			return GetByHistoryLine(historyLine, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryLineAsync(string historyLine, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryLine(string historyLine, int top_, TransactionManager tm_)
		{
			return GetByHistoryLine(historyLine, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryLineAsync(string historyLine, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryLine(string historyLine, string sort_)
		{
			return GetByHistoryLine(historyLine, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryLineAsync(string historyLine, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryLine(string historyLine, string sort_, TransactionManager tm_)
		{
			return GetByHistoryLine(historyLine, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryLineAsync(string historyLine, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryLine(string historyLine, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryLineAsync(string historyLine, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`HistoryLine` = @HistoryLine", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@HistoryLine", historyLine, MySqlDbType.VarChar));
		...
		/// /// <param name = "historyMultip">中奖线对应的倍率</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryMultip(string historyMultip)
		{
			return GetByHistoryMultip(historyMultip, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryMultipAsync(string historyMultip)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryMultip(string historyMultip, TransactionManager tm_)
		{
			return GetByHistoryMultip(historyMultip, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryMultipAsync(string historyMultip, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryMultip(string historyMultip, int top_)
		{
			return GetByHistoryMultip(historyMultip, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryMultipAsync(string historyMultip, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryMultip(string historyMultip, int top_, TransactionManager tm_)
		{
			return GetByHistoryMultip(historyMultip, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryMultipAsync(string historyMultip, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryMultip(string historyMultip, string sort_)
		{
			return GetByHistoryMultip(historyMultip, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryMultipAsync(string historyMultip, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryMultip(string historyMultip, string sort_, TransactionManager tm_)
		{
			return GetByHistoryMultip(historyMultip, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryMultipAsync(string historyMultip, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByHistoryMultip(string historyMultip, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByHistoryMultipAsync(string historyMultip, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`HistoryMultip` = @HistoryMultip", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@HistoryMultip", historyMultip, MySqlDbType.VarChar));
		...
		/// /// <param name = "rewardDetails">中奖详情</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRewardDetails(string rewardDetails)
		{
			return GetByRewardDetails(rewardDetails, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRewardDetailsAsync(string rewardDetails)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRewardDetails(string rewardDetails, TransactionManager tm_)
		{
			return GetByRewardDetails(rewardDetails, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRewardDetailsAsync(string rewardDetails, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRewardDetails(string rewardDetails, int top_)
		{
			return GetByRewardDetails(rewardDetails, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRewardDetailsAsync(string rewardDetails, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRewardDetails(string rewardDetails, int top_, TransactionManager tm_)
		{
			return GetByRewardDetails(rewardDetails, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRewardDetailsAsync(string rewardDetails, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRewardDetails(string rewardDetails, string sort_)
		{
			return GetByRewardDetails(rewardDetails, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRewardDetailsAsync(string rewardDetails, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRewardDetails(string rewardDetails, string sort_, TransactionManager tm_)
		{
			return GetByRewardDetails(rewardDetails, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRewardDetailsAsync(string rewardDetails, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRewardDetails(string rewardDetails, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByRewardDetailsAsync(string rewardDetails, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`RewardDetails` = @RewardDetails", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@RewardDetails", rewardDetails, MySqlDbType.VarChar));
		...
		/// /// <param name = "slotEleArray">返回给客户端数据结构</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetBySlotEleArray(string slotEleArray)
		{
			return GetBySlotEleArray(slotEleArray, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetBySlotEleArrayAsync(string slotEleArray)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetBySlotEleArray(string slotEleArray, TransactionManager tm_)
		{
			return GetBySlotEleArray(slotEleArray, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetBySlotEleArrayAsync(string slotEleArray, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetBySlotEleArray(string slotEleArray, int top_)
		{
			return GetBySlotEleArray(slotEleArray, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetBySlotEleArrayAsync(string slotEleArray, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetBySlotEleArray(string slotEleArray, int top_, TransactionManager tm_)
		{
			return GetBySlotEleArray(slotEleArray, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetBySlotEleArrayAsync(string slotEleArray, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetBySlotEleArray(string slotEleArray, string sort_)
		{
			return GetBySlotEleArray(slotEleArray, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetBySlotEleArrayAsync(string slotEleArray, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetBySlotEleArray(string slotEleArray, string sort_, TransactionManager tm_)
		{
			return GetBySlotEleArray(slotEleArray, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetBySlotEleArrayAsync(string slotEleArray, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetBySlotEleArray(string slotEleArray, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetBySlotEleArrayAsync(string slotEleArray, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`SlotEleArray` = @SlotEleArray", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@SlotEleArray", slotEleArray, MySqlDbType.VarChar));
		...
		/// /// <param name = "bonusInfo">特殊玩法数据结构</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBonusInfo(string bonusInfo)
		{
			return GetByBonusInfo(bonusInfo, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBonusInfoAsync(string bonusInfo)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBonusInfo(string bonusInfo, TransactionManager tm_)
		{
			return GetByBonusInfo(bonusInfo, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBonusInfoAsync(string bonusInfo, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBonusInfo(string bonusInfo, int top_)
		{
			return GetByBonusInfo(bonusInfo, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBonusInfoAsync(string bonusInfo, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBonusInfo(string bonusInfo, int top_, TransactionManager tm_)
		{
			return GetByBonusInfo(bonusInfo, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBonusInfoAsync(string bonusInfo, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBonusInfo(string bonusInfo, string sort_)
		{
			return GetByBonusInfo(bonusInfo, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBonusInfoAsync(string bonusInfo, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBonusInfo(string bonusInfo, string sort_, TransactionManager tm_)
		{
			return GetByBonusInfo(bonusInfo, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByBonusInfoAsync(string bonusInfo, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByBonusInfo(string bonusInfo, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByBonusInfoAsync(string bonusInfo, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`BonusInfo` = @BonusInfo", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@BonusInfo", bonusInfo, MySqlDbType.VarChar));
		...
		/// /// <param name = "userFlag">用户AB面</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserFlag(int userFlag)
		{
			return GetByUserFlag(userFlag, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserFlagAsync(int userFlag)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserFlag(int userFlag, TransactionManager tm_)
		{
			return GetByUserFlag(userFlag, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserFlagAsync(int userFlag, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserFlag(int userFlag, int top_)
		{
			return GetByUserFlag(userFlag, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserFlagAsync(int userFlag, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserFlag(int userFlag, int top_, TransactionManager tm_)
		{
			return GetByUserFlag(userFlag, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserFlagAsync(int userFlag, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserFlag(int userFlag, string sort_)
		{
			return GetByUserFlag(userFlag, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserFlagAsync(int userFlag, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserFlag(int userFlag, string sort_, TransactionManager tm_)
		{
			return GetByUserFlag(userFlag, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByUserFlagAsync(int userFlag, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByUserFlag(int userFlag, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByUserFlagAsync(int userFlag, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserFlag` = @UserFlag", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserFlag", userFlag, MySqlDbType.Int32));
		...
		/// /// <param name = "orderStatus">状态0-初始1-处理中2-成功3-失败4-已回滚5-异常且需处理6-异常已处理</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOrderStatus(int orderStatus)
		{
			return GetByOrderStatus(orderStatus, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOrderStatusAsync(int orderStatus)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOrderStatus(int orderStatus, TransactionManager tm_)
		{
			return GetByOrderStatus(orderStatus, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOrderStatus(int orderStatus, int top_)
		{
			return GetByOrderStatus(orderStatus, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOrderStatus(int orderStatus, int top_, TransactionManager tm_)
		{
			return GetByOrderStatus(orderStatus, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOrderStatus(int orderStatus, string sort_)
		{
			return GetByOrderStatus(orderStatus, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOrderStatus(int orderStatus, string sort_, TransactionManager tm_)
		{
			return GetByOrderStatus(orderStatus, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByOrderStatus(int orderStatus, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`OrderStatus` = @OrderStatus", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@OrderStatus", orderStatus, MySqlDbType.Int32));
		...
		/// /// <param name = "recDate">记录时间</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRecDate(DateTime recDate)
		{
			return GetByRecDate(recDate, 0, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRecDateAsync(DateTime recDate)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRecDate(DateTime recDate, TransactionManager tm_)
		{
			return GetByRecDate(recDate, 0, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRecDateAsync(DateTime recDate, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRecDate(DateTime recDate, int top_)
		{
			return GetByRecDate(recDate, top_, string.Empty, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRecDateAsync(DateTime recDate, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRecDate(DateTime recDate, int top_, TransactionManager tm_)
		{
			return GetByRecDate(recDate, top_, string.Empty, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRecDateAsync(DateTime recDate, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRecDate(DateTime recDate, string sort_)
		{
			return GetByRecDate(recDate, 0, sort_, null);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRecDateAsync(DateTime recDate, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRecDate(DateTime recDate, string sort_, TransactionManager tm_)
		{
			return GetByRecDate(recDate, 0, sort_, tm_);
		}
		public async Task<List<Fmo_bet_detailEO>> GetByRecDateAsync(DateTime recDate, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fmo_bet_detailEO> GetByRecDate(DateTime recDate, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fmo_bet_detailEO>> GetByRecDateAsync(DateTime recDate, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`RecDate` = @RecDate", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@RecDate", recDate, MySqlDbType.DateTime));
