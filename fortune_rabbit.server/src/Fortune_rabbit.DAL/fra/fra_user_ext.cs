...
namespace Fortune_rabbit.DAL
{
	...
	[DataContract]
	[Obsolete]
	public class Fra_user_extEO : IRowMapper<Fra_user_extEO>
	{
		/// <summary>
		/// 构造函数 
		/// </summary>
		public Fra_user_extEO()
		{
			this.UserFlag = 0;
		...
		/// <return>实体对象</return>
		public Fra_user_extEO MapRow(IDataReader reader)
		{
		...
		/// <param name = "reader">只进结果集流</param>
		/// <return>实体对象</return>
		public static Fra_user_extEO MapDataReader(IDataReader reader)
		{
		    Fra_user_extEO ret = new Fra_user_extEO();
	...
	/// </summary>
	[Obsolete]
	public class Fra_user_extMO : MySqlTableMO<Fra_user_extEO>
	{
		/// <summary>
		/// 表名
		/// </summary>
	    public override string TableName { get; set; } = "`fra_user_ext`";
		...
		public Fra_user_extMO(MySqlDatabase database) : base(database) { }
		/// <summary>
		/// 构造函数
		/// </summary>
		/// <param name = "connectionStringName">配置文件.config中定义的连接字符串名称</param>
		public Fra_user_extMO(string connectionStringName = null) : base(connectionStringName) { }
	    /// <summary>
	    /// 构造函数
	    /// </summary>
	    /// <param name="connectionString">数据库连接字符串，如server=192.168.1.1;database=testdb;uid=root;pwd=root</param>
	    /// <param name="commandTimeout">CommandTimeout时间</param>
	    public Fra_user_extMO(string connectionString, int commandTimeout) : base(connectionString, commandTimeout) { }
		...
		public override int Add(Fra_user_extEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_); 
		}
		public override async Task<int> AddAsync(Fra_user_extEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_); 
		}
	    private void RepairAddData(Fra_user_extEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = useIgnore_ ? "INSERT IGNORE" : "INSERT";
			sql_ += $" INTO {TableName} (`UserID`, `AppID`, `OperatorID`, `CurrencyID`, `UserFlag`, `HistoryBetCount`, `HistoryWinAmount`, `LastRecDate`) VALUE (@UserID, @AppID, @OperatorID, @CurrencyID, @UserFlag, @HistoryBetCount, @HistoryWinAmount, @LastRecDate);";
		...
		public int AddByBatch(IEnumerable<Fra_user_extEO> items, int batchCount, TransactionManager tm_ = null)
		{
			var ret = 0;
	    ...
	    public async Task<int> AddByBatchAsync(IEnumerable<Fra_user_extEO> items, int batchCount, TransactionManager tm_ = null)
	    {
	        var ret = 0;
	        foreach (var sql in BuildAddBatchSql(items, batchCount))
	        {
	    ...
	    private IEnumerable<string> BuildAddBatchSql(IEnumerable<Fra_user_extEO> items, int batchCount)
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`UserID`, `AppID`, `OperatorID`, `CurrencyID`, `UserFlag`, `HistoryBetCount`, `HistoryWinAmount`, `LastRecDate`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"('{item.UserID}','{item.AppID}','{item.OperatorID}','{item.CurrencyID}',{item.UserFlag},{item.HistoryBetCount},{item.HistoryWinAmount},'{item.LastRecDate?.ToString("yyyy-MM-dd HH:mm:ss")}'),");
				if (count == batchCount)
				{
					count = 0;
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Remove(Fra_user_extEO item, TransactionManager tm_ = null)
		{
			return RemoveByPK(item.UserID, tm_);
		}
		public async Task<int> RemoveAsync(Fra_user_extEO item, TransactionManager tm_ = null)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(Fra_user_extEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAsync(Fra_user_extEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutData(Fra_user_extEO item, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `UserID` = @UserID, `AppID` = @AppID, `OperatorID` = @OperatorID, `CurrencyID` = @CurrencyID, `UserFlag` = @UserFlag, `HistoryBetCount` = @HistoryBetCount, `HistoryWinAmount` = @HistoryWinAmount WHERE `UserID` = @UserID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@UserID", item.UserID, MySqlDbType.VarChar),
				Database.CreateInParameter("@AppID", item.AppID, MySqlDbType.VarChar),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(IEnumerable<Fra_user_extEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public async Task<int> PutAsync(IEnumerable<Fra_user_extEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public bool Set(Fra_user_extEO item, TransactionManager tm = null)
		{
			bool ret = true;
		...
		public async Task<bool> SetAsync(Fra_user_extEO item, TransactionManager tm = null)
		{
			bool ret = true;
			if(GetByPK(item.UserID) == null)
			{
		...
		/// <return></return>
		public Fra_user_extEO GetByPK(string userID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(userID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return Database.ExecSqlSingle(sql_, paras_, tm_, Fra_user_extEO.MapDataReader);
		}
		public async Task<Fra_user_extEO> GetByPKAsync(string userID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(userID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return await Database.ExecSqlSingleAsync(sql_, paras_, tm_, Fra_user_extEO.MapDataReader);
		}
		...
		public List<Fra_user_extEO> GetByAppID(string appID)
		{
			return GetByAppID(appID, 0, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByAppIDAsync(string appID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByAppID(string appID, TransactionManager tm_)
		{
			return GetByAppID(appID, 0, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByAppIDAsync(string appID, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByAppID(string appID, int top_)
		{
			return GetByAppID(appID, top_, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByAppIDAsync(string appID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByAppID(string appID, int top_, TransactionManager tm_)
		{
			return GetByAppID(appID, top_, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByAppIDAsync(string appID, int top_, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByAppID(string appID, string sort_)
		{
			return GetByAppID(appID, 0, sort_, null);
		}
		public async Task<List<Fra_user_extEO>> GetByAppIDAsync(string appID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByAppID(string appID, string sort_, TransactionManager tm_)
		{
			return GetByAppID(appID, 0, sort_, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByAppIDAsync(string appID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByAppID(string appID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`AppID` = @AppID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@AppID", appID, MySqlDbType.VarChar));
			return Database.ExecSqlList(sql_, paras_, tm_, Fra_user_extEO.MapDataReader);
		}
		public async Task<List<Fra_user_extEO>> GetByAppIDAsync(string appID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`AppID` = @AppID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@AppID", appID, MySqlDbType.VarChar));
			return await Database.ExecSqlListAsync(sql_, paras_, tm_, Fra_user_extEO.MapDataReader);
		}
		...
		/// /// <param name = "operatorID">运营商编码</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByOperatorID(string operatorID)
		{
			return GetByOperatorID(operatorID, 0, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByOperatorIDAsync(string operatorID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByOperatorID(string operatorID, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, 0, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByOperatorIDAsync(string operatorID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByOperatorID(string operatorID, int top_)
		{
			return GetByOperatorID(operatorID, top_, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByOperatorIDAsync(string operatorID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByOperatorID(string operatorID, int top_, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, top_, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByOperatorIDAsync(string operatorID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByOperatorID(string operatorID, string sort_)
		{
			return GetByOperatorID(operatorID, 0, sort_, null);
		}
		public async Task<List<Fra_user_extEO>> GetByOperatorIDAsync(string operatorID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByOperatorID(string operatorID, string sort_, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, 0, sort_, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByOperatorIDAsync(string operatorID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByOperatorID(string operatorID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fra_user_extEO>> GetByOperatorIDAsync(string operatorID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`OperatorID` = @OperatorID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@OperatorID", operatorID, MySqlDbType.VarChar));
		...
		/// /// <param name = "currencyID">货币编码（ISO 4217大写）</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByCurrencyID(string currencyID)
		{
			return GetByCurrencyID(currencyID, 0, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByCurrencyIDAsync(string currencyID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByCurrencyID(string currencyID, TransactionManager tm_)
		{
			return GetByCurrencyID(currencyID, 0, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByCurrencyIDAsync(string currencyID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByCurrencyID(string currencyID, int top_)
		{
			return GetByCurrencyID(currencyID, top_, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByCurrencyIDAsync(string currencyID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByCurrencyID(string currencyID, int top_, TransactionManager tm_)
		{
			return GetByCurrencyID(currencyID, top_, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByCurrencyIDAsync(string currencyID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByCurrencyID(string currencyID, string sort_)
		{
			return GetByCurrencyID(currencyID, 0, sort_, null);
		}
		public async Task<List<Fra_user_extEO>> GetByCurrencyIDAsync(string currencyID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByCurrencyID(string currencyID, string sort_, TransactionManager tm_)
		{
			return GetByCurrencyID(currencyID, 0, sort_, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByCurrencyIDAsync(string currencyID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByCurrencyID(string currencyID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fra_user_extEO>> GetByCurrencyIDAsync(string currencyID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`CurrencyID` = @CurrencyID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@CurrencyID", currencyID, MySqlDbType.VarChar));
		...
		/// /// <param name = "userFlag">用户标识</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByUserFlag(long userFlag)
		{
			return GetByUserFlag(userFlag, 0, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByUserFlagAsync(long userFlag)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByUserFlag(long userFlag, TransactionManager tm_)
		{
			return GetByUserFlag(userFlag, 0, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByUserFlagAsync(long userFlag, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByUserFlag(long userFlag, int top_)
		{
			return GetByUserFlag(userFlag, top_, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByUserFlagAsync(long userFlag, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByUserFlag(long userFlag, int top_, TransactionManager tm_)
		{
			return GetByUserFlag(userFlag, top_, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByUserFlagAsync(long userFlag, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByUserFlag(long userFlag, string sort_)
		{
			return GetByUserFlag(userFlag, 0, sort_, null);
		}
		public async Task<List<Fra_user_extEO>> GetByUserFlagAsync(long userFlag, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByUserFlag(long userFlag, string sort_, TransactionManager tm_)
		{
			return GetByUserFlag(userFlag, 0, sort_, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByUserFlagAsync(long userFlag, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByUserFlag(long userFlag, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fra_user_extEO>> GetByUserFlagAsync(long userFlag, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserFlag` = @UserFlag", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserFlag", userFlag, MySqlDbType.Int64));
		...
		/// /// <param name = "historyBetCount">历史总下注手数</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryBetCount(int historyBetCount)
		{
			return GetByHistoryBetCount(historyBetCount, 0, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryBetCountAsync(int historyBetCount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryBetCount(int historyBetCount, TransactionManager tm_)
		{
			return GetByHistoryBetCount(historyBetCount, 0, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryBetCountAsync(int historyBetCount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryBetCount(int historyBetCount, int top_)
		{
			return GetByHistoryBetCount(historyBetCount, top_, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryBetCountAsync(int historyBetCount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryBetCount(int historyBetCount, int top_, TransactionManager tm_)
		{
			return GetByHistoryBetCount(historyBetCount, top_, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryBetCountAsync(int historyBetCount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryBetCount(int historyBetCount, string sort_)
		{
			return GetByHistoryBetCount(historyBetCount, 0, sort_, null);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryBetCountAsync(int historyBetCount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryBetCount(int historyBetCount, string sort_, TransactionManager tm_)
		{
			return GetByHistoryBetCount(historyBetCount, 0, sort_, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryBetCountAsync(int historyBetCount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryBetCount(int historyBetCount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fra_user_extEO>> GetByHistoryBetCountAsync(int historyBetCount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`HistoryBetCount` = @HistoryBetCount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@HistoryBetCount", historyBetCount, MySqlDbType.Int32));
		...
		/// /// <param name = "historyWinAmount">历史总盈亏</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryWinAmount(long historyWinAmount)
		{
			return GetByHistoryWinAmount(historyWinAmount, 0, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryWinAmountAsync(long historyWinAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryWinAmount(long historyWinAmount, TransactionManager tm_)
		{
			return GetByHistoryWinAmount(historyWinAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryWinAmountAsync(long historyWinAmount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryWinAmount(long historyWinAmount, int top_)
		{
			return GetByHistoryWinAmount(historyWinAmount, top_, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryWinAmountAsync(long historyWinAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryWinAmount(long historyWinAmount, int top_, TransactionManager tm_)
		{
			return GetByHistoryWinAmount(historyWinAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryWinAmountAsync(long historyWinAmount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryWinAmount(long historyWinAmount, string sort_)
		{
			return GetByHistoryWinAmount(historyWinAmount, 0, sort_, null);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryWinAmountAsync(long historyWinAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryWinAmount(long historyWinAmount, string sort_, TransactionManager tm_)
		{
			return GetByHistoryWinAmount(historyWinAmount, 0, sort_, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByHistoryWinAmountAsync(long historyWinAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByHistoryWinAmount(long historyWinAmount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fra_user_extEO>> GetByHistoryWinAmountAsync(long historyWinAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`HistoryWinAmount` = @HistoryWinAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@HistoryWinAmount", historyWinAmount, MySqlDbType.Int64));
		...
		/// /// <param name = "lastRecDate">记录最后记录时间</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByLastRecDate(DateTime? lastRecDate)
		{
			return GetByLastRecDate(lastRecDate, 0, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByLastRecDate(DateTime? lastRecDate, TransactionManager tm_)
		{
			return GetByLastRecDate(lastRecDate, 0, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByLastRecDate(DateTime? lastRecDate, int top_)
		{
			return GetByLastRecDate(lastRecDate, top_, string.Empty, null);
		}
		public async Task<List<Fra_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByLastRecDate(DateTime? lastRecDate, int top_, TransactionManager tm_)
		{
			return GetByLastRecDate(lastRecDate, top_, string.Empty, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByLastRecDate(DateTime? lastRecDate, string sort_)
		{
			return GetByLastRecDate(lastRecDate, 0, sort_, null);
		}
		public async Task<List<Fra_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByLastRecDate(DateTime? lastRecDate, string sort_, TransactionManager tm_)
		{
			return GetByLastRecDate(lastRecDate, 0, sort_, tm_);
		}
		public async Task<List<Fra_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Fra_user_extEO> GetByLastRecDate(DateTime? lastRecDate, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Fra_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL(lastRecDate.HasValue ? "`LastRecDate` = @LastRecDate" : "`LastRecDate` IS NULL", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			if (lastRecDate.HasValue)
