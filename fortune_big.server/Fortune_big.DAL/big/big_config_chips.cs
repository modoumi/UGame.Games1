...
namespace Fortune_big.DAL
{
	...
	/// <summary>
	/// 下注筹码表
	/// 【表 big_config_chips 的实体类】
	/// </summary>
	[DataContract]
	[Obsolete]
	public class Big_config_chipsEO : IRowMapper<Big_config_chipsEO>
	{
		/// <summary>
		/// 构造函数 
		/// </summary>
		public Big_config_chipsEO()
		{
			this.BetAmount = 0;
			this.SingleAmount = 0;
			this.LineNum = 0;
		...
		/// <return>实体对象</return>
		public Big_config_chipsEO MapRow(IDataReader reader)
		{
		...
		/// <param name = "reader">只进结果集流</param>
		/// <return>实体对象</return>
		public static Big_config_chipsEO MapDataReader(IDataReader reader)
		{
		    Big_config_chipsEO ret = new Big_config_chipsEO();
	...
	/// <summary>
	/// 下注筹码表
	/// 【表 big_config_chips 的操作类】
	/// </summary>
	[Obsolete]
	public class Big_config_chipsMO : MySqlTableMO<Big_config_chipsEO>
	{
		/// <summary>
		/// 表名
		/// </summary>
	    public override string TableName { get; set; } = "`big_config_chips`";
		...
		public Big_config_chipsMO(MySqlDatabase database) : base(database) { }
		/// <summary>
		/// 构造函数
		/// </summary>
		/// <param name = "connectionStringName">配置文件.config中定义的连接字符串名称</param>
		public Big_config_chipsMO(string connectionStringName = null) : base(connectionStringName) { }
	    /// <summary>
	    /// 构造函数
	    /// </summary>
	    /// <param name="connectionString">数据库连接字符串，如server=192.168.1.1;database=testdb;uid=root;pwd=root</param>
	    /// <param name="commandTimeout">CommandTimeout时间</param>
	    public Big_config_chipsMO(string connectionString, int commandTimeout) : base(connectionString, commandTimeout) { }
		...
		public override int Add(Big_config_chipsEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			item.ChipsID = Database.ExecSqlScalar<int>(sql_, paras_, tm_); 
			return 1;
		}
		public override async Task<int> AddAsync(Big_config_chipsEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			item.ChipsID = await Database.ExecSqlScalarAsync<int>(sql_, paras_, tm_); 
	        return 1;
		}
	    private void RepairAddData(Big_config_chipsEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = useIgnore_ ? "INSERT IGNORE" : "INSERT";
			sql_ += $" INTO {TableName} (`BetAmount`, `SingleAmount`, `LineNum`, `IsShortcut`) VALUE (@BetAmount, @SingleAmount, @LineNum, @IsShortcut);SELECT LAST_INSERT_ID();";
		...
		public int AddByBatch(IEnumerable<Big_config_chipsEO> items, int batchCount, TransactionManager tm_ = null)
		{
			var ret = 0;
	    ...
	    public async Task<int> AddByBatchAsync(IEnumerable<Big_config_chipsEO> items, int batchCount, TransactionManager tm_ = null)
	    {
	        var ret = 0;
	        foreach (var sql in BuildAddBatchSql(items, batchCount))
	        {
	    ...
	    private IEnumerable<string> BuildAddBatchSql(IEnumerable<Big_config_chipsEO> items, int batchCount)
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`ChipsID`, `BetAmount`, `SingleAmount`, `LineNum`, `IsShortcut`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"({item.ChipsID},{item.BetAmount},{item.SingleAmount},{item.LineNum},{item.IsShortcut}),");
				if (count == batchCount)
				{
					count = 0;
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Remove(Big_config_chipsEO item, TransactionManager tm_ = null)
		{
			return RemoveByPK(item.ChipsID, tm_);
		}
		public async Task<int> RemoveAsync(Big_config_chipsEO item, TransactionManager tm_ = null)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(Big_config_chipsEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAsync(Big_config_chipsEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutData(Big_config_chipsEO item, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `BetAmount` = @BetAmount, `SingleAmount` = @SingleAmount, `LineNum` = @LineNum, `IsShortcut` = @IsShortcut WHERE `ChipsID` = @ChipsID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@BetAmount", item.BetAmount, MySqlDbType.Int64),
				Database.CreateInParameter("@SingleAmount", item.SingleAmount, MySqlDbType.Int64),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(IEnumerable<Big_config_chipsEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public async Task<int> PutAsync(IEnumerable<Big_config_chipsEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public bool Set(Big_config_chipsEO item, TransactionManager tm = null)
		{
			bool ret = true;
		...
		public async Task<bool> SetAsync(Big_config_chipsEO item, TransactionManager tm = null)
		{
			bool ret = true;
			if(GetByPK(item.ChipsID) == null)
			{
		...
		/// <return></return>
		public Big_config_chipsEO GetByPK(int chipsID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(chipsID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return Database.ExecSqlSingle(sql_, paras_, tm_, Big_config_chipsEO.MapDataReader);
		}
		public async Task<Big_config_chipsEO> GetByPKAsync(int chipsID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(chipsID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return await Database.ExecSqlSingleAsync(sql_, paras_, tm_, Big_config_chipsEO.MapDataReader);
		}
		...
		public List<Big_config_chipsEO> GetByBetAmount(long betAmount)
		{
			return GetByBetAmount(betAmount, 0, string.Empty, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByBetAmountAsync(long betAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByBetAmount(long betAmount, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByBetAmountAsync(long betAmount, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByBetAmount(long betAmount, int top_)
		{
			return GetByBetAmount(betAmount, top_, string.Empty, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByBetAmountAsync(long betAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByBetAmount(long betAmount, int top_, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByBetAmountAsync(long betAmount, int top_, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByBetAmount(long betAmount, string sort_)
		{
			return GetByBetAmount(betAmount, 0, sort_, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByBetAmountAsync(long betAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByBetAmount(long betAmount, string sort_, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, 0, sort_, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByBetAmountAsync(long betAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByBetAmount(long betAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`BetAmount` = @BetAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@BetAmount", betAmount, MySqlDbType.Int64));
			return Database.ExecSqlList(sql_, paras_, tm_, Big_config_chipsEO.MapDataReader);
		}
		public async Task<List<Big_config_chipsEO>> GetByBetAmountAsync(long betAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`BetAmount` = @BetAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@BetAmount", betAmount, MySqlDbType.Int64));
			return await Database.ExecSqlListAsync(sql_, paras_, tm_, Big_config_chipsEO.MapDataReader);
		}
		...
		/// /// <param name = "singleAmount">单线下注金额</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetBySingleAmount(long singleAmount)
		{
			return GetBySingleAmount(singleAmount, 0, string.Empty, null);
		}
		public async Task<List<Big_config_chipsEO>> GetBySingleAmountAsync(long singleAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetBySingleAmount(long singleAmount, TransactionManager tm_)
		{
			return GetBySingleAmount(singleAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetBySingleAmountAsync(long singleAmount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetBySingleAmount(long singleAmount, int top_)
		{
			return GetBySingleAmount(singleAmount, top_, string.Empty, null);
		}
		public async Task<List<Big_config_chipsEO>> GetBySingleAmountAsync(long singleAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetBySingleAmount(long singleAmount, int top_, TransactionManager tm_)
		{
			return GetBySingleAmount(singleAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetBySingleAmountAsync(long singleAmount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetBySingleAmount(long singleAmount, string sort_)
		{
			return GetBySingleAmount(singleAmount, 0, sort_, null);
		}
		public async Task<List<Big_config_chipsEO>> GetBySingleAmountAsync(long singleAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetBySingleAmount(long singleAmount, string sort_, TransactionManager tm_)
		{
			return GetBySingleAmount(singleAmount, 0, sort_, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetBySingleAmountAsync(long singleAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetBySingleAmount(long singleAmount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Big_config_chipsEO>> GetBySingleAmountAsync(long singleAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`SingleAmount` = @SingleAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@SingleAmount", singleAmount, MySqlDbType.Int64));
		...
		/// /// <param name = "lineNum">线数</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByLineNum(int lineNum)
		{
			return GetByLineNum(lineNum, 0, string.Empty, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByLineNumAsync(int lineNum)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByLineNum(int lineNum, TransactionManager tm_)
		{
			return GetByLineNum(lineNum, 0, string.Empty, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByLineNumAsync(int lineNum, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByLineNum(int lineNum, int top_)
		{
			return GetByLineNum(lineNum, top_, string.Empty, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByLineNumAsync(int lineNum, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByLineNum(int lineNum, int top_, TransactionManager tm_)
		{
			return GetByLineNum(lineNum, top_, string.Empty, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByLineNumAsync(int lineNum, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByLineNum(int lineNum, string sort_)
		{
			return GetByLineNum(lineNum, 0, sort_, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByLineNumAsync(int lineNum, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByLineNum(int lineNum, string sort_, TransactionManager tm_)
		{
			return GetByLineNum(lineNum, 0, sort_, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByLineNumAsync(int lineNum, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByLineNum(int lineNum, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Big_config_chipsEO>> GetByLineNumAsync(int lineNum, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`LineNum` = @LineNum", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@LineNum", lineNum, MySqlDbType.Int32));
		...
		/// /// <param name = "isShortcut">是否出现在+-</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByIsShortcut(int isShortcut)
		{
			return GetByIsShortcut(isShortcut, 0, string.Empty, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByIsShortcutAsync(int isShortcut)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByIsShortcut(int isShortcut, TransactionManager tm_)
		{
			return GetByIsShortcut(isShortcut, 0, string.Empty, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByIsShortcutAsync(int isShortcut, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByIsShortcut(int isShortcut, int top_)
		{
			return GetByIsShortcut(isShortcut, top_, string.Empty, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByIsShortcutAsync(int isShortcut, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByIsShortcut(int isShortcut, int top_, TransactionManager tm_)
		{
			return GetByIsShortcut(isShortcut, top_, string.Empty, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByIsShortcutAsync(int isShortcut, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByIsShortcut(int isShortcut, string sort_)
		{
			return GetByIsShortcut(isShortcut, 0, sort_, null);
		}
		public async Task<List<Big_config_chipsEO>> GetByIsShortcutAsync(int isShortcut, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByIsShortcut(int isShortcut, string sort_, TransactionManager tm_)
		{
			return GetByIsShortcut(isShortcut, 0, sort_, tm_);
		}
		public async Task<List<Big_config_chipsEO>> GetByIsShortcutAsync(int isShortcut, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Big_config_chipsEO> GetByIsShortcut(int isShortcut, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Big_config_chipsEO>> GetByIsShortcutAsync(int isShortcut, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`IsShortcut` = @IsShortcut", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@IsShortcut", isShortcut, MySqlDbType.Int32));
