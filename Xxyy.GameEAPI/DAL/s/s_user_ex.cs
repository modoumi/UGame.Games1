...
namespace Xxyy.DAL
{
    ...
    public class S_user_exMO : MySqlTableMO<S_user_exEO>
    {
        ...
        private void RepairAddData(S_user_exEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
        {
            ...
            sql_ += $" INTO {TableName} (`UserID`, `UserKind`, `OperatorID`, `CountryID`, `CurrencyID`, `TotalBonus`, `TotalBonusCount`, `TotalChangeAmount`, `TotalChangeCount`, `TotalBetAmount`, `TotalBetCount`, `TotalWinAmount`, `TotalWinCount`, `TotalPayAmount`, `TotalPayCount`, `TotalCashAmount`, `TotalCashCount`, `FirstPayAmount`) VALUE (@UserID, @UserKind, @OperatorID, @CountryID, @CurrencyID, @TotalBonus, @TotalBonusCount, @TotalChangeAmount, @TotalChangeCount, @TotalBetAmount, @TotalBetCount, @TotalWinAmount, @TotalWinCount, @TotalPayAmount, @TotalPayCount, @TotalCashAmount, @TotalCashCount, @FirstPayAmount);";
        ...
        {
            var count = 0;
            var insertSql = $"INSERT INTO {TableName} (`UserID`, `UserKind`, `OperatorID`, `CountryID`, `CurrencyID`, `TotalBonus`, `TotalBonusCount`, `TotalChangeAmount`, `TotalChangeCount`, `TotalBetAmount`, `TotalBetCount`, `TotalWinAmount`, `TotalWinCount`, `TotalPayAmount`, `TotalPayCount`, `TotalCashAmount`, `TotalCashCount`, `FirstPayAmount`) VALUES ";
            var sql = new StringBuilder();
            foreach (var item in items)
            {
                count++;
                sql.Append($"('{item.UserID}',{item.UserKind},'{item.OperatorID}','{item.CountryID}','{item.CurrencyID}',{item.TotalBonus},{item.TotalBonusCount},{item.TotalChangeAmount},{item.TotalChangeCount},{item.TotalBetAmount},{item.TotalBetCount},{item.TotalWinAmount},{item.TotalWinCount},{item.TotalPayAmount},{item.TotalPayCount},{item.TotalCashAmount},{item.TotalCashCount},{item.FirstPayAmount}),");
                if (count == batchCount)
                {
                    count = 0;
        ...
        private void RepairGetByPKData(string userID, out string sql_, out List<MySqlParameter> paras_, bool isForUpdate_ = false)
        {
