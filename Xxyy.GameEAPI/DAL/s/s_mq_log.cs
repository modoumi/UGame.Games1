...
namespace Xxyy.DAL
{
    ...
    public class S_mq_logMO : MySqlTableMO<S_mq_logEO>
    {
        ...
        private void RepairAddData(S_mq_logEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
        {
            ...
            sql_ += $" INTO {TableName} (`MQLogID`, `ConsumerType`, `MessageType`, `MessageData`, `MessageId`, `ActionName`, `Exception`, `ProjectId`, `MessageLevel`, `Status`, `RecDate`, `RepublishDate`) VALUE (@MQLogID, @ConsumerType, @MessageType, @MessageData, @MessageId, @ActionName, @Exception, @ProjectId, @MessageLevel, @Status, @RecDate, @RepublishDate);";
        ...
        {
            var count = 0;
            var insertSql = $"INSERT INTO {TableName} (`MQLogID`, `ConsumerType`, `MessageType`, `MessageData`, `MessageId`, `ActionName`, `Exception`, `ProjectId`, `MessageLevel`, `Status`, `RecDate`, `RepublishDate`) VALUES ";
            var sql = new StringBuilder();
            foreach (var item in items)
            {
                count++;
                sql.Append($"('{item.MQLogID}','{item.ConsumerType}','{item.MessageType}','{item.MessageData}','{item.MessageId}','{item.ActionName}','{item.Exception}','{item.ProjectId}',{item.MessageLevel},{item.Status},'{item.RecDate.ToString("yyyy-MM-dd HH:mm:ss")}','{item.RepublishDate?.ToString("yyyy-MM-dd HH:mm:ss")}'),");
                if (count == batchCount)
                {
                    count = 0;
        ...
        {
            sql_ = $"UPDATE {TableName} SET `MQLogID` = @MQLogID, `ConsumerType` = @ConsumerType, `MessageType` = @MessageType, `MessageData` = @MessageData, `MessageId` = @MessageId, `ActionName` = @ActionName, `Exception` = @Exception, `ProjectId` = @ProjectId, `MessageLevel` = @MessageLevel, `Status` = @Status WHERE `MQLogID` = @MQLogID_Original";
            paras_ = new List<MySqlParameter>()
            {
                Database.CreateInParameter("@MQLogID", item.MQLogID, MySqlDbType.VarChar),
                Database.CreateInParameter("@ConsumerType", item.ConsumerType != null ? item.ConsumerType : (object)DBNull.Value, MySqlDbType.VarChar),
        ...
        {
            RepairGetByPKData(mQLogID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_);
        ...
        private void RepairGetByPKData(string mQLogID, out string sql_, out List<MySqlParameter> paras_, bool isForUpdate_ = false)
        {
