...
namespace Xxyy.DAL
{
    ...
    public class S_operatorMO : MySqlTableMO<S_operatorEO>
    {
        ...
        {
            var count = 0;
            var insertSql = $"INSERT INTO {TableName} (`OperatorID`, `OperatorBaseID`, `CountryID`, `Langs`, `CurrencyID`, `MapDomain`, `LobbyUrl`, `LobbyUrlStaging`, `LobbyUrlDebug`, `BankUrl`, `BankUrlStaging`, `BankUrlDebug`, `BranchKey`, `GAKey`, `Title`, `Note`, `EnableBonus`, `ChangeBalanceMode`, `FlowMode`, `FirstPayFlowMultip`, `PayFlowMultip`, `FlowBalance`, `FlowDevideBalance`, `Status`, `RecDate`) VALUES ";
            var sql = new StringBuilder();
            foreach (var item in items)
            {
                count++;
                sql.Append($"('{item.OperatorID}','{item.OperatorBaseID}','{item.CountryID}','{item.Langs}','{item.CurrencyID}','{item.MapDomain}','{item.LobbyUrl}','{item.LobbyUrlStaging}','{item.LobbyUrlDebug}','{item.BankUrl}','{item.BankUrlStaging}','{item.BankUrlDebug}','{item.BranchKey}','{item.GAKey}','{item.Title}','{item.Note}',{item.EnableBonus},{item.ChangeBalanceMode},{item.FlowMode},{item.FirstPayFlowMultip},{item.PayFlowMultip},{item.FlowBalance},{item.FlowDevideBalance},{item.Status},'{item.RecDate.ToString("yyyy-MM-dd HH:mm:ss")}'),");
                if (count == batchCount)
                {
                    count = 0;
        ...
        /// 按字段删除
        /// </summary>
        /// /// <param name = "flowMode">流水计算方式（不同的执行类）1-仅流水+充值2-</param>
        /// <param name="tm_">事务管理对象</param>
        ...
        {
            RepairGetByPKData(operatorID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_);
        ...
        private void RepairGetByPKData(string operatorID, out string sql_, out List<MySqlParameter> paras_, bool isForUpdate_ = false)
        {
