...
namespace Xxyy.DAL
{
    ...
    public class Sc_cash_auditMO : MySqlTableMO<Sc_cash_auditEO>
    {
        ...
        private void RepairAddData(Sc_cash_auditEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
        {
            ...
            sql_ += $" INTO {TableName} (`CashAuditID`, `UserID`, `UserKind`, `FromMode`, `FromId`, `AppID`, `OperatorID`, `CountryID`, `CurrencyID`, `Mobile`, `Amount`, `BankCode`, `BankName`, `AccName`, `AccNumber`, `Channels`, `Status`, `ApplyTime`, `AuditTime`, `OperatorUser`, `BankID`, `Paytype`, `Channel`, `RequestParam`, `Remark`) VALUE (@CashAuditID, @UserID, @UserKind, @FromMode, @FromId, @AppID, @OperatorID, @CountryID, @CurrencyID, @Mobile, @Amount, @BankCode, @BankName, @AccName, @AccNumber, @Channels, @Status, @ApplyTime, @AuditTime, @OperatorUser, @BankID, @Paytype, @Channel, @RequestParam, @Remark);";
        ...
        {
            var count = 0;
            var insertSql = $"INSERT INTO {TableName} (`CashAuditID`, `UserID`, `UserKind`, `FromMode`, `FromId`, `AppID`, `OperatorID`, `CountryID`, `CurrencyID`, `Mobile`, `Amount`, `BankCode`, `BankName`, `AccName`, `AccNumber`, `Channels`, `Status`, `ApplyTime`, `AuditTime`, `OperatorUser`, `BankID`, `Paytype`, `Channel`, `RequestParam`, `Remark`) VALUES ";
            var sql = new StringBuilder();
            foreach (var item in items)
            {
                count++;
                sql.Append($"('{item.CashAuditID}','{item.UserID}',{item.UserKind},{item.FromMode},'{item.FromId}','{item.AppID}','{item.OperatorID}','{item.CountryID}','{item.CurrencyID}','{item.Mobile}',{item.Amount},'{item.BankCode}','{item.BankName}','{item.AccName}','{item.AccNumber}',{item.Channels},{item.Status},'{item.ApplyTime.ToString("yyyy-MM-dd HH:mm:ss")}','{item.AuditTime?.ToString("yyyy-MM-dd HH:mm:ss")}','{item.OperatorUser}','{item.BankID}',{item.Paytype},{item.Channel},'{item.RequestParam}','{item.Remark}'),");
                if (count == batchCount)
                {
                    count = 0;
        ...
        {
            sql_ = $"UPDATE {TableName} SET `CashAuditID` = @CashAuditID, `UserID` = @UserID, `UserKind` = @UserKind, `FromMode` = @FromMode, `FromId` = @FromId, `AppID` = @AppID, `OperatorID` = @OperatorID, `CountryID` = @CountryID, `CurrencyID` = @CurrencyID, `Mobile` = @Mobile, `Amount` = @Amount, `BankCode` = @BankCode, `BankName` = @BankName, `AccName` = @AccName, `AccNumber` = @AccNumber, `Channels` = @Channels, `Status` = @Status, `AuditTime` = @AuditTime, `OperatorUser` = @OperatorUser, `BankID` = @BankID, `Paytype` = @Paytype, `Channel` = @Channel, `RequestParam` = @RequestParam, `Remark` = @Remark WHERE `CashAuditID` = @CashAuditID_Original";
            paras_ = new List<MySqlParameter>()
            {
                Database.CreateInParameter("@CashAuditID", item.CashAuditID, MySqlDbType.VarChar),
                Database.CreateInParameter("@UserID", item.UserID, MySqlDbType.VarChar),
        ...
        {
            RepairGetByPKData(cashAuditID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_);
        ...
        private void RepairGetByPKData(string cashAuditID, out string sql_, out List<MySqlParameter> paras_, bool isForUpdate_ = false)
        {
