...
namespace Xxyy.GameEAPI.Common
{
    public class AppSectionBase : ConfigSection
    {
        ...
        public override void Bind(IConfiguration configuration)
        {
            ...
            {
                if (string.IsNullOrEmpty(ServerId))
                    ServerId = StringUtil.GetGuidString();
            }
            ...
            {
                if (string.IsNullOrEmpty(ServerUrl)) //配置文件
                {
                    ServerUrl = GetServerUrlFromApp(); //s_app.ServerUrl
                }
            }
        }
        private string GetServerUrlFromApp()
        {
            ...
            switch (ServerType)
            {
                case ServerType.Staging:
                    return app.ServerUrlStaging;
                case ServerType.JointDebug:
                    return app.ServerUrlDebug;
                default:
